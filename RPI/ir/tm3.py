	...
	for width in (640, 560, 480, 360):
		...
		if found is None or highestMaxVal > 0:
			...
			if tempResult[0] > threshold:               
				...
				cv2.putText(image, templatesName[templateIndex], (maxLoc[0]-3, maxLoc[1]-3), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 2, cv2.LINE_AA)
				cv2.rectangle(image,(int(maxLoc[0]*r), int(maxLoc[1]*r)), (int((maxLoc[0]+tW)*r), int((maxLoc[1]+tH)*r)), (0, 0, 255), 2)
